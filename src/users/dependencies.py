...
def get_service(db: Annotated[AsyncSession, Depends(get_db)]) -> UserService:
    """
    Provides a UserService instance with a database session dependency.

    Args:
        db (AsyncSession, optional): Database session provided by dependency injection.

    Returns:
        UserService: An instance of UserService initialized with the given session.
    """
...
async def get_user_from_uuid(
    ...
    Args:
        ...
        service (UserService, optional): Dependency-injected UserService instance.
